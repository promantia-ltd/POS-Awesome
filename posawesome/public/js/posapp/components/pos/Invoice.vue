      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message) {
            ...
            // Apply customer discount only if customer_info exists and has a valid discount
            if (
            ...
            if (!item.batch_price) {
              if (
                ...
                // Set rate to price_list_rate if no customer discount applies
      ...
      if (this.customer) {
        frappe.call({
          ...
          callback: (r) => {
            ...
            vm.update_price_list();
            // Update existing items with new customer details
        ...
        // Reset customer_info when no customer is selected
        vm.customer_info = { posa_discount: 0 };
        // Update existing items to remove any customer-specific discounts
        vm.items.forEach((item) => {
          vm.update_item_detail(item);
        });
      }
    ...
    calc_prices(item, value, $event) {
      ...
      if ($event?.target?.id === "rate" || $event?.target?.id === "gridRate") {
        ...
        if (newValue < 0) {
          ...
          item.discount_percentage = this.flt((newValue / item.price_list_rate) * 100, this.float_precision);
        }
