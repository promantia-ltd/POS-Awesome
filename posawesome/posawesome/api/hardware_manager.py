...
def get_enhanced_context(doc):
	...
	return {
		...
		"format_qty": lambda x: "{:.2f}".format(flt(x)),
...
def hardware_url(api_name):
	...
	api_endpoint = next(
		(item.api_endpoint for item in doc.api_details if item.api_name == api_name),
		None
	)
...
def get_print_templates(doctype=None):
	...
	templates = frappe.get_all(
		...
		filters=filters,
		fields=["name", "print_format", "ref_doctype"]
	)
...
def generate_print_xml(doc_type, sales_invoice_name, template_path=None, template_name=None):
	...
	if template_name:
		...
		template_content = frappe.get_value(
			"POS XML Print Designer",
			template_name,
			"xml_template"
		)
		...
		default_template = frappe.get_value(
			"POS XML Print Designer",
			{"ref_doctype": doc_type, "is_default": 1},
			"xml_template"
		)

		if not default_template:
			frappe.throw(_("No default template found for {0}. Please set a template as default or specify template_name.").format(doc_type))
...
def validate_jinja_syntax(template):
	...
	if len(open_blocks) != len(close_blocks):
		errors.append(f"Unmatched Jinja blocks: {len(open_blocks)} opening tags, {len(close_blocks)} closing tags")
...
def validate_xml_template(xml_template, doc_type):
	...
	if doc_type:
		try:
			...
			warnings.append(f"Could not validate fields: {str(e)}")
