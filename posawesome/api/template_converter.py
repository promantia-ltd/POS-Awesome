...
class HTMLToXMLConverter:
		...
		for child in element.children:
			if isinstance(child, Tag):
				...
				if text:
					...
					attr_str = ' ' + ' '.join(f'{k}="{v}"' for k, v in attrs.items()) if attrs else ''
					lines.append(' ' * indent + f'<line><text{attr_str}>{self._escape_xml(text)}</text></line>')
	...
	def _convert_italic(self, element, indent):
		...
		self.warnings.append("Italic text detected - thermal printers don't support italic, using regular text")
	...
	def _convert_table(self, element, indent):
		...
		for row_idx, row in enumerate(rows):
			...
			for cell_idx, cell in enumerate(cells):
				...
				line_parts.append(f'<text align="{align}" length="{col_width}"{bold_attr}>{{{{ truncate({cell_text}, {col_width}) }}}}</text>')
